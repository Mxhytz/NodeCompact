...
-- @param fillVal type Description
...
function TArray:get(i)
	return self._b[i] or 0
end
--- TArray:set
-- @param i type Description
-- @param v type Description
-- @return type Description
function TArray:set(i, v)
	self._b[i] = v
end
--- TArray:clone
-- @return type Description
...
function Pool:releaseArray(a)
	local n = a.n
	if not self._arrays[n] then
		self._arrays[n] = {}
	end
	ins(self._arrays[n], a)
end
--- Pool:acquireMat
-- @param r type Description
...
--- incRC
-- @param rc type Description
-- @return type Description
local function incRC(rc)
	rc.v = rc.v + 1
end
--- decRC
-- @param rc type Description
-- @return type Description
local function decRC(rc)
	rc.v = rc.v - 1;
	return rc.v
end
--- isShared
...
function Vec.from( ...)
	...
	if type(a[1]) == "table" and not getmetatable(a[1]) then
		return Vec.fromTable(a[1])
	end
...
function Vec.randn(n, mu, sig)
	...
	local v = Vec.new(n)
	for i = 1, n do
		v._d:set(i, mu + sig * sqrt(- 2 * log(rand())) * cos(2 * pi * rand()))
	end
	return v
end
--- Vec.basis
...
--- Vec:toTable
-- @return type Description
function Vec:toTable()
	return self._d:toTable()
end
--- Vec:add_
-- @param b type Description
-- @return type Description
...
function Vec:sub_(b)
	...
	if type(b) == "number" then
		for i = 1, self.n do
			self._d:set(i, self._d:get(i) - b)
		end
	else
		for i = 1, self.n do
			self._d:set(i, self._d:get(i) - b._d:get(i))
		end
	end;
	return self
end
...
-- @return type Description
function Vec:neg_()
	return self:scale_(- 1)
end
--- Vec:map_
-- @param f type Description
-- @return type Description
function Vec:map_(f)
	self:_cow()
	for i = 1, self.n do
		self._d:set(i, f(self._d:get(i), i))
	end;
	return self
end
--- Vec:clamp_
-- @param lo type Description
-- @param hi type Description
-- @return type Description
...
-- @return type Description
function Vec:sub(b)
	return self:clone():sub_(b)
end
--- Vec:scale
-- @param s type Description
-- @return type Description
function Vec:scale(s)
	return self:clone():scale_(s)
end
--- Vec:neg
-- @return type Description
function Vec:neg()
	return self:clone():scale_(- 1)
end
--- Vec:map
-- @param f type Description
-- @return type Description
function Vec:map(f)
	return self:clone():map_(f)
end
--- Vec:hadamard
-- @param b type Description
-- @return type Description
function Vec:hadamard(b)
	...
	for i = 1, self.n do
		v._d:set(i, self._d:get(i) * b._d:get(i))
	end;
	return v
end
--- Vec:dot
...
function Vec:norm(p)
	...
	if p == huge then
		local m = 0;
		for i = 1, self.n do
			m = math.max(m, abs(self._d:get(i)))
		end;
		return m
	end
	local s = 0;
	for i = 1, self.n do
		s = s + abs(self._d:get(i)) ^ p
	end;
	return s ^ (1 / p)
end
--- Vec:norm2
...
function Vec:normalize()
	local n = self:norm2();
	return n < EPS and self:clone() or self:scale(1 / n)
end
--- Vec:normalize_
-- @return type Description
...
-- @return type Description
function Vec:dist(b)
	return self:sub(b):norm2()
end
--- Vec:cross
-- @param b type Description
-- @return type Description
function Vec:cross(b)
	return Vec.from(self._d:get(2) * b._d:get(3) - self._d:get(3) * b._d:get(2), self._d:get(3) * b._d:get(1) - self._d:get(1) * b._d:get(3), self._d:get(1) * b._d:get(2) - self._d:get(2) * b._d:get(1))
end
--- Vec:angle
-- @param b type Description
-- @return type Description
...
-- @param b type Description
-- @return type Description
function Vec:project(b)
	return b:scale(self:dot(b) / b:dot(b))
end
--- Vec:reject
-- @param b type Description
-- @return type Description
function Vec:reject(b)
	return self:sub(self:project(b))
end
--- Vec:lerp
-- @param b type Description
-- @param t type Description
...
function Vec:lerp(b, t)
	return self:scale(1 - t):add_(b:scale(t))
end
--- Vec:sum
-- @return type Description
function Vec:sum()
...
function Vec:slice(a, b)
	a = a or 1;
	b = b or self.n;
	local v = Vec.new(b - a + 1)
	for i = a, b do
		v._d:set(i - a + 1, self._d:get(i))
	end;
	return v
end
--- Vec:concat
...
function Vec:concat(b)
	...
	for i = 1, self.n do
		v._d:set(i, self._d:get(i))
	end
	for i = 1, b.n do
		v._d:set(self.n + i, b._d:get(i))
	end;
	return v
end
--- Vec:sort
...
function Vec:sort(f)
	...
	table.sort(t, f);
	return Vec.fromTable(t)
end
--- Vec:eq
-- @param b type Description
-- @param eps type Description
-- @return type Description
function Vec:eq(b, eps)
	...
	for i = 1, self.n do
		if abs(self._d:get(i) - b._d:get(i)) > eps then
			return false
		end
	end;
	return true
...
function Vec:linspace_(a, b)
	for i = 1, self.n do
		self._d:set(i, a + (b - a) * (i - 1) / (self.n - 1))
	end;
	return self
end
--- Vec:quantile
...
-- @param b type Description
-- @return type Description
function Vec:cosineSim(b)
	return self:dot(b) / (self:norm2() * b:norm2() + EPS)
end
--- Vec:toMat
-- @param rows type Description
-- @param cols type Description
-- @return type Description
...
function Mat.fromTable(data)
	...
	for i = 1, r do
		for j = 1, c do
			m._d:set((i - 1) * c + j, data[i][j] or 0)
		end
	end;
	return m
end
...
-- @param c type Description
-- @return type Description
function Mat.zeros(r, c)
	return Mat.new(r, c, 0)
end
--- Mat.ones
-- @param r type Description
-- @param c type Description
-- @return type Description
function Mat.ones(r, c)
	return Mat.new(r, c, 1)
end
--- Mat.rand
-- @param r type Description
-- @param c type Description
...
function Mat.randn(r, c, mu, sig)
	...
	local n = r * c
	for i = 1, n do
		m._d:set(i, mu + sig * sqrt(- 2 * log(rand())) * cos(2 * pi * rand()))
	end;
	return m
end
--- Mat.diag
...
--- Mat:idx
-- @param i type Description
-- @param j type Description
-- @return type Description
function Mat:idx(i, j)
...
function Mat:get(i, j)
	return self._d:get((i - 1) * self.cols + j)
end
--- Mat:set
-- @param i type Description
-- @param j type Description
...
function Mat:hadamard(b)
	...
	local n = self.rows * self.cols
	for i = 1, n do
		m._d:set(i, m._d:get(i) * b._d:get(i))
	end;
	return m
end
--- Mat:T
...
function Mat:row(i)
	...
	for j = 1, self.cols do
		v._d:set(j, self._d:get((i - 1) * self.cols + j))
	end;
	return v
end
--- Mat:col
...
function Mat:col(j)
	...
	for i = 1, self.rows do
		v._d:set(i, self._d:get((i - 1) * self.cols + j))
	end;
	return v
end
--- Mat:setRow
...
function Mat:setRow(i, v)
	self:_cow()
	for j = 1, self.cols do
		self._d:set((i - 1) * self.cols + j, v._d:get(j))
	end;
	return self
end
--- Mat:setCol
...
function Mat:setCol(j, v)
	self:_cow()
	for i = 1, self.rows do
		self._d:set((i - 1) * self.cols + j, v._d:get(i))
	end;
	return self
end
--- Mat:submat
-- @param r1 type Description
-- @param r2 type Description
-- @param c1 type Description
-- @param c2 type Description
-- @return type Description
...
function Mat:reshape(r, c)
	local m = Mat._wrap(self._d, r, c, self._rc);
	incRC(self._rc);
	return m
end
--- Mat:flatten
...
function Mat:toTable()
	...
	for i = 1, self.rows do
		r[i] = {}
		for j = 1, self.cols do
			r[i][j] = self._d:get((i - 1) * self.cols + j)
		end
	end;
	return r
end
...
function Mat:trace()
	local s = 0;
	local mn = math.min(self.rows, self.cols)
	for i = 1, mn do
		s = s + self._d:get((i - 1) * self.cols + i)
	end;
	return s
end
--- Mat:frobenius
...
--- Mat:rowSums
-- @return type Description
function Mat:rowSums()
	local v = Vec.new(self.rows);
	for i = 1, self.rows do
		v._d:set(i, self:row(i):sum())
	end;
	return v
end
--- Mat:colSums
-- @return type Description
function Mat:colSums()
	local v = Vec.new(self.cols);
	for j = 1, self.cols do
		v._d:set(j, self:col(j):sum())
	end;
	return v
end
--- Mat:eq
...
function Mat:__tostring()
	...
	for i = 1, self.rows do
		local row = {}
		for j = 1, self.cols do
			row[j] = fmt("%10.5g", self:get(i, j))
		end
		lines[i] = "[ " .. tcat(row, "  ") .. " ]"
	end;
	return tcat(lines, "\n")
end
Mat.__add = function(a, b)
	if type(a) == "number" then
		return b:add(a)
	end;
	return a:add(b)
end
Mat.__sub = function(a, b)
	return a:sub(b)
end
Mat.__mul = function(a, b)
	if type(a) == "number" then
		return b:scale(a)
	end;
	return a:mul(b)
end
Mat.__unm = function(a)
	return a:neg()
end
Mat.__eq = function(a, b)
	return a:eq(b)
end
--- Mat.fromRows
-- @param vs type Description
-- @return type Description
function Mat.fromRows(vs)
	local m = Mat.new(# vs, vs[1].n)
	for i, v in ipairs(vs) do
		for j = 1, v.n do
			m:set(i, j, v._d:get(j))
		end
	end;
	return m
end
...
function Mat:pow(p)
	...
	if p == 0 then
		return Mat.identity(self.rows)
	end
	if p == 1 then
		return self:clone()
	end
	if p % 2 == 0 then
		local h = self:pow(math.floor(p / 2));
		return h:mul(h)
	end
...
function Mat:isSymmetric(eps)
	...
	for i = 1, self.rows do
		for j = i + 1, self.cols do
			if abs(self:get(i, j) - self:get(j, i)) > eps then
				return false
			end
		end
	end
...
function Mat:isDiag(eps)
	...
	for i = 1, self.rows do
		for j = 1, self.cols do
			if i ~= j and abs(self:get(i, j)) > eps then
				return false
			end
		end
	end
...
-- @param t type Description
-- @return type Description
function mathx.mat(t)
	return Mat.fromTable(t)
end
local blas = {}
--- blas.dscal
-- @param x type Description
-- @param alpha type Description
-- @return type Description
...
-- @param T type Description
-- @param b type Description
-- @param lower type Description
-- @param unit type Description
-- @return type Description
...
-- @param c type Description
-- @return type Description
local function unpad(M, r, c)
	return M:submat(1, r, 1, c)
end
--- splitMat
-- @param M type Description
-- @return type Description
local function splitMat(M)
	...
	return M:submat(1, h, 1, h), M:submat(1, h, h + 1, M.cols), M:submat(h + 1, M.rows, 1, h), M:submat(h + 1, M.rows, h + 1, M.cols)
end
--- joinMat
-- @param C11 type Description
-- @param C12 type Description
-- @param C21 type Description
-- @param C22 type Description
-- @return type Description
...
function blas.strassen(A, B)
	local Ap, origR, origC = padPow2(A);
	local Bp = padPow2(B)
	local Cp = strassenRec(Ap, Bp);
	return unpad(Cp, origR, B.cols)
end
--- blas.winograd
-- @param A type Description
-- @param B type Description
...
function blas.winograd(A, B)
	...
	for i = 1, m do
		...
		for j = 1, n do
			...
			for p = 1, k_half do
				local p2 = p * 2
				s = s + (a[(i - 1) * k + p2 - 1] + b[(p2 - 2) * n + j]) * (a[(i - 1) * k + p2] + b[(p2 - 1) * n + j])
			end
			if k % 2 == 1 then
...
function lapack.dgetrf(A)
	...
	for k = 1, n do
		...
		if maxr ~= k then
			sgn = - sgn;
			P[k], P[maxr] = P[maxr], P[k]
			for j = 1, n do
				local t = U._d:get((k - 1) * n + j);
				U._d:set((k - 1) * n + j, U._d:get((maxr - 1) * n + j));
				U._d:set((maxr - 1) * n + j, t)
			end
			for j = 1, k - 1 do
				local t = L._d:get((k - 1) * n + j);
				L._d:set((k - 1) * n + j, L._d:get((maxr - 1) * n + j));
				L._d:set((maxr - 1) * n + j, t)
			end
		end
		for i = k + 1, n do
			local f = U:get(i, k) / U:get(k, k);
			L:set(i, k, f)
			for j = k, n do
...
function lapack.det(A)
	local _, U, _, sgn = lapack.dgetrf(A);
	local d = sgn
	for i = 1, U.rows do
		d = d * U:get(i, i)
	end;
...
function lapack.dgeqrf(A)
	...
	for j = 1, n do
		local v = A:col(j)
		for i = 1, j - 1 do
			local rij = blas.ddot(qs[i], v);
			R:set(i, j, rij);
			blas.daxpy(qs[i], - rij, v)
		end
		local nrm = blas.dnrm2(v);
		R:set(j, j, nrm)
		qs[j] = nrm > EPS and v:scale(1 / nrm) or Vec.new(m)
		for i = 1, m do
			Q:set(i, j, qs[j]._d:get(i))
		end
	end;
	return Q, R
end
...
function lapack.dsyev(A)
	...
	if n <= 20 then
		...
		table.sort(idx, function(a, b)
			return vals._d:get(a) > vals._d:get(b)
		end)
...
function lapack.dgesvd(A)
	...
	table.sort(idx, function(a, b)
		return math.abs(d._d:get(a)) > math.abs(d._d:get(b))
	end)
	local S = Vec.new(n)
...
function lapack.dpotrf(A)
	...
	for i = 1, n do
		for j = 1, i do
			local s = A:get(i, j)
			for p = 1, j - 1 do
				s = s - L:get(i, p) * L:get(j, p)
			end
			if i == j then
				L:set(i, j, sqrt(math.max(0, s)))
			else
				local ljj = L:get(j, j);
				L:set(i, j, ljj > EPS and s / ljj or 0)
			end
		end
	end;
	return L
...
--- lapack.pinv
-- @param A type Description
-- @param tol type Description
-- @return type Description
function lapack.pinv(A, tol)
	local U, S, V = lapack.dgesvd(A);
	tol = tol or EPS;
	local k = S.n
...
-- @param b type Description
-- @return type Description
function lapack.lstsq(A, b)
	return lapack.pinv(A):mul(b)
end
--- lapack.cond
-- @param A type Description
-- @return type Description
function lapack.cond(A)
	local _, S = lapack.dgesvd(A);
	return S._d:get(1) / (S._d:get(S.n) + EPS)
end
--- lapack.rank
-- @param A type Description
-- @param tol type Description
...
function lapack.null(A, tol)
	...
	for col = 1, M.cols do
		local pivR = nil;
		for i = row, M.rows do
			if abs(M:get(i, col)) > tol then
				pivR = i;
				break
			end
		end
		if pivR then
			pivCols[# pivCols + 1] = col
			for j = 1, M.cols do
				local t = M:get(row, j);
				M:set(row, j, M:get(pivR, j));
				M:set(pivR, j, t)
			end
			local p = M:get(row, col);
			for j = 1, M.cols do
				M:set(row, j, M:get(row, j) / p)
			end
			for i = 1, M.rows do
				if i ~= row then
					local f = M:get(i, col)
					for j = 1, M.cols do
						M:set(i, j, M:get(i, j) - f * M:get(row, j))
					end
				end
			end
			row = row + 1
	...
	local pivSet = {};
	for _, c in ipairs(pivCols) do
		pivSet[c] = true
	end
	local free = {};
	for j = 1, M.cols do
		if not pivSet[j] then
			free[# free + 1] = j
		end
	end
	local basis = {}
	for _, fc in ipairs(free) do
		local v = Vec.new(M.cols);
		v._d:set(fc, 1)
		for pi, pc in ipairs(pivCols) do
			v._d:set(pc, - M:get(pi, fc))
		end;
		basis[# basis + 1] = v
	end
	return basis
...
--- lapack.expm
-- @param A type Description
-- @param terms type Description
-- @return type Description
function lapack.expm(A, terms)
	terms = terms or 20;
	local n = A.rows;
	local R = Mat.identity(n);
	local Ak = Mat.identity(n);
	local fk = 1
	for k = 1, terms do
		fk = fk * k;
		Ak = blas.dgemm(Ak, A);
		R:add_(Ak:scale(1 / fk))
	end;
	return R
end
mathx.lapack = lapack
...
la.solve = function(A, b)
	local L, U, P = lapack.dgetrf(A);
	return lapack.dgetrs(L, U, P, b)
end
la.solveM = function(A, B)
	local X = Mat.new(A.rows, B.cols)
	for j = 1, B.cols do
		local col = la.solve(A, B:col(j));
		for i = 1, A.rows do
			X:set(i, j, col._d:get(i))
		end
	end;
...
la.norm = function(A, k)
	if k == "fro" or not k then
		return A:frobenius()
	end
	if k == 1 then
		local m = 0;
		for j = 1, A.cols do
			local s = A:col(j):map(function(v)
				return v < 0 and - v or v
			end):sum();
			if s > m then
				m = s
	...
	if k == huge then
		local m = 0;
		for i = 1, A.rows do
			local s = A:row(i):map(function(v)
				return v < 0 and - v or v
			end):sum();
			if s > m then
				m = s
...
la.gramSchmidt = function(vs)
	...
	for _, v in ipairs(vs) do
		...
		for _, q in ipairs(r) do
			blas.daxpy(q, - blas.ddot(q, v), u)
		end
		local n = blas.dnrm2(u);
		if n > EPS then
			r[# r + 1] = u:scale(1 / n)
		end
	end;
...
-- @param x type Description
-- @return type Description
function fp.fromFloat(x)
	return floor(x * FP_ONE + 0.5)
end
--- fp.toFloat
-- @param x type Description
-- @return type Description
function fp.toFloat(x)
	return x / FP_ONE
end
--- fp.add
-- @param a type Description
...
-- @return type Description
function fp.sqrt_(a)
	return fp.fromFloat(sqrt(fp.toFloat(a)))
end
--- fp.floor_
-- @param a type Description
-- @return type Description
function fp.floor_(a)
	return floor(a / FP_ONE) * FP_ONE
end
--- fp.ceil_
-- @param a type Description
-- @return type Description
function fp.ceil_(a)
	return ceil(a / FP_ONE) * FP_ONE
end
--- fp.lerp_
-- @param a type Description
-- @param b type Description
-- @param t type Description
-- @return type Description
function fp.lerp_(a, b, t)
	return a + fp.div(fp.mul(b - a, t), FP_ONE)
end
--- fp.dot
-- @param as type Description
-- @param bs type Description
-- @return type Description
function fp.dot(as, bs)
	local s = 0;
	for i = 1, # as do
		s = s + fp.mul(as[i], bs[i])
	end;
	return s
end
--- fp.Vec
...
function fp.addSat(a, b)
	local r = a + b;
	return r > FP_MAX and FP_MAX or r < FP_MIN and FP_MIN or r
end
--- fp.mulSat
-- @param a type Description
-- @param b type Description
...
function pp.dgemm_f32(A, B)
	...
	for i = 1, m do
		for j = 1, n do
			local s = 0
			for p = 1, k do
				s = toF32(s + toF32(toF32(A:get(i, p)) * toF32(B:get(p, j))))
			end
			C:set(i, j, s)
		end
	end;
...
-- @param A type Description
-- @param b type Description
-- @param maxIter type Description
-- @param tol type Description
-- @return type Description
function pp.iterRefine(A, b, maxIter, tol)
	tol = tol or EPS;
	maxIter = maxIter or 5
	local L, U, P = lapack.dgetrf(A)
...
--- pp.eigProgressive
-- @param A type Description
-- @param roughIters type Description
-- @param refineIters type Description
-- @param tol type Description
-- @return type Description
function pp.eigProgressive(A, roughIters, refineIters, tol)
	roughIters = roughIters or 10;
	refineIters = refineIters or 1000
	tol = tol or EPS;
	local n = A.rows
	local Ak = A:clone();
	local V = Mat.identity(n)
	for _ = 1, roughIters do
		local Q, R = lapack.dgeqrf(Ak);
		Ak = blas.dgemm(R, Q);
		V = blas.dgemm(V, Q)
	end
...
-- @param n type Description
-- @return type Description
function smx.hilbert(n)
	return Mat.new(n, n):map_(function(_, i, j)
		return 1 / (i + j - 1)
	end)
end
--- smx.vandermonde
...
--- smx.toeplitz
-- @param row type Description
-- @param col type Description
-- @return type Description
function smx.toeplitz(row, col)
	...
	for i = 1, r do
		for j = 1, c do
			local d = j - i;
			m:set(i, j, d >= 0 and row[d + 1] or col[- d + 1])
		end
	end;
	return m
end
...
function smx.hadamardMat(n)
	if n == 1 then
		return Mat.fromTable({
			{
				1
			}
		})
	end
	local H = smx.hadamardMat(math.floor(n / 2));
	local m = Mat.new(n, n)
	for i = 1, math.floor(n / 2) do
		for j = 1, math.floor(n / 2) do
			local v = H:get(i, j)
			m:set(i, j, v);
			m:set(i, j + math.floor(n / 2), v);
			m:set(i + math.floor(n / 2), j, v);
			m:set(i + math.floor(n / 2), j + math.floor(n / 2), - v)
		end
	end;
	return m
end
...
function smx.kron(A, B)
	...
	for i = 1, A.rows do
		for j = 1, A.cols do
			...
			for p = 1, B.rows do
				for q = 1, B.cols do
					m:set((i - 1) * B.rows + p, (j - 1) * B.cols + q, a * B:get(p, q))
				end
			end
		end
	end;
...
function smx.blockDiag(mats)
	...
	local B = Mat.new(tr, tc);
	local roff, coff = 0, 0
	for _, M in ipairs(mats) do
		for i = 1, M.rows do
			for j = 1, M.cols do
				B:set(roff + i, coff + j, M:get(i, j))
			end
		end
		roff = roff + M.rows;
		coff = coff + M.cols
...
--- smx.rotation2d
-- @param th type Description
-- @return type Description
function smx.rotation2d(th)
	return Mat.fromTable({
		{
			cos(th),
			- sin(th)
...
function smx.householder(v)
	...
	for i = 1, n do
		for j = 1, n do
			m:set(i, j, (i == j and 1 or 0) - 2 * vn._d:get(i) * vn._d:get(j))
		end
	end;
	return m
end
...
function smx.givens(n, i, j, th)
	local G = Mat.identity(n);
	local c2, s2 = cos(th), sin(th)
	G:set(i, i, c2);
	G:set(j, j, c2);
	G:set(i, j, - s2);
	G:set(j, i, s2);
...
function smx.pascal(n)
	local m = Mat.new(n, n)
--- ch
-- @param nn type Description
-- @param kk type Description
-- @return type Description
	local function ch(nn, kk)
...
function calc.partial(f, x, i, h)
	h = h or 1e-7;
	local xp = x:clone();
	xp._d:set(i, xp._d:get(i) + h)
	local xm = x:clone();
	xm._d:set(i, xm._d:get(i) - h);
...
function calc.grad(f, x, h)
	...
	local g = Vec.new(x.n)
	for i = 1, x.n do
		g._d:set(i, calc.partial(f, x, i, h))
	end;
	return g
end
--- calc.jacobian
...
function calc.jacobian(f, x, h)
	h = h or 1e-7;
	local fx = f(x);
	local J = Mat.new(fx.n, x.n)
	for j = 1, x.n do
		local xp = x:clone();
		xp._d:set(j, xp._d:get(j) + h)
		local xm = x:clone();
		xm._d:set(j, xm._d:get(j) - h)
		local col = f(xp):sub(f(xm)):scale_(1 / (2 * h))
		for i = 1, fx.n do
			J:set(i, j, col._d:get(i))
		end
	end;
	return J
end
...
function calc.hessian(f, x, h)
	...
	for i = 1, n do
		for j = 1, n do
			local pp2 = x:clone();
			pp2._d:set(i, pp2._d:get(i) + h);
			pp2._d:set(j, pp2._d:get(j) + h)
			local pm = x:clone();
			pm._d:set(i, pm._d:get(i) + h);
			pm._d:set(j, pm._d:get(j) - h)
			local mp = x:clone();
			mp._d:set(i, mp._d:get(i) - h);
			mp._d:set(j, mp._d:get(j) + h)
			local mm = x:clone();
			mm._d:set(i, mm._d:get(i) - h);
			mm._d:set(j, mm._d:get(j) - h)
			H:set(i, j, (f(pp2) - f(pm) - f(mp) + f(mm)) / (4 * h * h))
		end
	end;
	return H
end
...
function calc.gaussLegendre(f, a, b)
	...
	local s = 0
	for i = 1, 8 do
		s = s + GL8[2][i] * f(mid + half * GL8[1][i])
	end;
	return s * half
end
--- calc.simpson
...
--- calc.rk4
-- @param f type Description
-- @param t0 type Description
-- @param y0 type Description
-- @param tf type Description
-- @param n type Description
-- @return type Description
...
--- A
-- @param a type Description
-- @param b2 type Description
-- @param s type Description
-- @return type Description
	local function A(a, b2, s)
		return type(a) == "number" and a + b2 * s or a:add(b2:scale(s))
	end
	local t, y = t0, y0;
	local ts, ys = {
		t0
	...
	for _ = 1, n do
		local k1 = f(t, y);
		local k2 = f(t + h / 2, A(y, k1, h / 2))
		local k3 = f(t + h / 2, A(y, k2, h / 2));
		local k4 = f(t + h, A(y, k3, h))
		if type(y) == "number" then
			y = y + (k1 + 2 * k2 + 2 * k3 + k4) * h / 6
		else
			y = y:add(k1:add(k2:scale(2)):add(k3:scale(2)):add(k4):scale(h / 6))
		end
		t = t + h;
		ts[# ts + 1] = t;
		ys[# ys + 1] = y
...
function calc.newton(f, df, x0, tol, maxI)
	...
	for _ = 1, maxI do
		...
		x = x - (fx / (df and df(x) or calc.diff(f, x)))
	end;
	return x, f(x)
end
--- calc.secant
...
function calc.secant(f, x0, x1, tol, maxI)
	...
	maxI = maxI or 100
	for _ = 1, maxI do
		local f0, f1 = f(x0), f(x1);
		if abs(f1) < tol then
			return x1, f1
		end
		local dx = f1 * (x1 - x0) / (f1 - f0);
		x0, x1 = x1, x1 - dx;
		if abs(dx) < tol then
			return x1, f(x1)
		end
...
function calc.newtonVec(F, x0, tol, maxI)
	...
	for _ = 1, maxI do
		local Fx = F(x);
		if Fx:norm2() < tol then
			return x
		end
		local J = calc.jacobian(F, x);
		local dx = la.solve(J, Fx:neg());
		x:add_(dx)
		if dx:norm2() < tol then
			return x
		end
	end;
	return x
...
function calc.nelderMead(f, x0, maxI, tol)
	...
	for i = 1, n do
		local xi = x0:clone();
		xi._d:set(i, xi._d:get(i) + (abs(xi._d:get(i)) < 1e-10 and 0.00025 or 0.05));
		S[i + 1] = xi
	end
	for _ = 1, maxI do
		local vals = {};
		...
		table.sort(vals, function(a2, b2)
			return a2[1] < b2[1]
		end)
		if vals[n + 1][1] - vals[1][1] < tol then
			break
		end
		local cen = Vec.new(n);
		for i = 1, n do
		...
		cen:scale_(1 / n)
		local xw = S[vals[n + 1][2]];
		local xr = cen:add(cen:sub(xw));
		local fr = f(xr)
		if fr < vals[1][1] then
			local xe = cen:add(cen:sub(xw):scale(2));
			S[vals[n + 1][2]] = f(xe) < fr and xe or xr
		elseif fr < vals[n][1] then
			S[vals[n + 1][2]] = xr
		else
			local xc = cen:add(xw:sub(cen):scale(0.5))
			if f(xc) < vals[n + 1][1] then
				S[vals[n + 1][2]] = xc
			else
				local x1 = S[vals[1][2]]
				for i = 2, n + 1 do
					S[vals[i][2]] = x1:add(S[vals[i][2]]:sub(x1):scale(0.5))
				end
			end
		end
	end
...
-- @param C type Description
-- @return type Description
function Poly:integ(C)
	C = C or 0;
	local nc = {
		C
	};
	for i = 1, self.deg + 1 do
		nc[i + 1] = self.c[i] / i
	end;
	return Poly.new(nc)
end
...
function Poly:add(b)
	local nc = {};
	for i = 1, math.max(self.deg, b.deg) + 1 do
		nc[i] = (self.c[i] or 0) + (b.c[i] or 0)
	end;
	return Poly.new(nc)
end
--- Poly:mul
...
function Poly:mul(b)
	local nc = {};
	for i = 1, self.deg + b.deg + 2 do
		nc[i] = 0
	end
	for i = 1, self.deg + 1 do
		for j = 1, b.deg + 1 do
			nc[i + j - 1] = nc[i + j - 1] + self.c[i] * b.c[j]
		end
	end;
	return Poly.new(nc)
end
...
-- @param b2 type Description
-- @return type Description
function Poly:integrate(a, b2)
	return self:integ():eval(b2) - self:integ():eval(a)
end
--- Poly:roots
-- @return type Description
function Poly:roots()
...
function Poly:__tostring()
	local parts = {};
	for i = self.deg + 1, 1, - 1 do
		local cv = self.c[i];
		local e = i - 1
		if abs(cv) > 1e-14 then
			if e == 0 then
				parts[# parts + 1] = fmt("%.6g", cv)
			elseif e == 1 then
				parts[# parts + 1] = fmt("%.6gx", cv)
			else
				parts[# parts + 1] = fmt("%.6gx^%d", cv, e)
			end
		end
	end
...
-- @param t type Description
-- @return type Description
function calc.poly(t)
	return Poly.new(t)
end
--- calc.cubicSpline
-- @param xs type Description
-- @param ys type Description
-- @return type Description
function calc.cubicSpline(xs, ys)
	...
	local b2, d = {}, {}
	for j = n, 1, - 1 do
		cn[j] = z[j] - mu[j] * (cn[j + 1] or 0)
		b2[j] = (ys[j + 1] - ys[j]) / h[j] - h[j] * ((cn[j + 1] or 0) + 2 * cn[j]) / 3
		d[j] = ((cn[j + 1] or 0) - cn[j]) / (3 * h[j])
	end
	return function(x)
		for i = 1, n do
			if x <= xs[i + 1] or i == n then
				local dx = x - xs[i];
				return ys[i] + b2[i] * dx + cn[i] * dx ^ 2 + d[i] * dx ^ 3
			end
		end
	end
end
...
-- @return type Description
function Cx:add(b)
	return Cx.new(self.r + b.r, self.i + b.i)
end
--- Cx:sub
-- @param b type Description
-- @return type Description
...
-- @return type Description
function Cx:mul(b)
	return Cx.new(self.r * b.r - self.i * b.i, self.r * b.i + self.i * b.r)
end
--- Cx:scale
-- @param s type Description
-- @return type Description
function Cx:scale(s)
	return Cx.new(self.r * s, self.i * s)
end
--- Cx:conj
-- @return type Description
function Cx:conj()
	return Cx.new(self.r, - self.i)
end
--- Cx:mag
-- @return type Description
function Cx:mag()
...
local function fftIP(a, inv)
	...
	for k = 1, math.floor(n / 2) do
		...
		local tr = wr * or2 - wi * oi;
		local ti = wr * oi + wi * or2
		a[k] = Cx.new(er + tr, ei + ti);
		a[k + math.floor(n / 2)] = Cx.new(er - tr, ei - ti)
		local nwr = wr * wnr - wi * wni;
		wi = wr * wni + wi * wnr;
		wr = nwr
	end
end
...
function fft.fft(xs)
	...
	local a = {}
	for i = 1, n do
		a[i] = type(xs[i]) == "number" and Cx.new(xs[i]) or (xs[i] or Cx.new(0))
	end
	fftIP(a, false);
	return a
end
...
--- fft.freq
-- @param n type Description
-- @param sr type Description
-- @return type Description
function fft.freq(n, sr)
	sr = sr or 1;
	local v = Vec.new(n);
	for i = 1, n do
		v._d:set(i, (i - 1) * sr / n)
...
function fft.convolve(a, b)
	...
	local fa, fb = {}, {}
	for i = 1, n do
		fa[i] = Cx.new(a[i] or 0);
		fb[i] = Cx.new(b[i] or 0)
	end
	fftIP(fa, false);
	fftIP(fb, false)
	local fc = {};
...
-- @param b type Description
-- @return type Description
function fft.xcorr(a, b)
	local bb = {};
	for i = # b, 1, - 1 do
		bb[# bb + 1] = b[i]
	end;
	return fft.convolve(a, bb)
end
--- fft.autocorr
-- @param a type Description
-- @return type Description
function fft.autocorr(a)
	return fft.xcorr(a, a)
end
--- fft.window
-- @param kind type Description
-- @param n type Description
-- @return type Description
function fft.window(kind, n)
	...
	for i = 1, n do
		local t = (i - 1) / (n - 1)
		if kind == "hann" then
			w._d:set(i, 0.5 - 0.5 * cos(2 * pi * t))
		elseif kind == "hamming" then
			w._d:set(i, 0.54 - 0.46 * cos(2 * pi * t))
		elseif kind == "blackman" then
			w._d:set(i, 0.42 - 0.5 * cos(2 * pi * t) + 0.08 * cos(4 * pi * t))
		elseif kind == "bartlett" then
			w._d:set(i, 1 - abs(2 * t - 1))
		else
			w._d:set(i, 1)
		end
	end;
...
--- fft.stft
-- @param xs type Description
-- @param winSize type Description
-- @param hop type Description
-- @param winKind type Description
-- @return type Description
function fft.stft(xs, winSize, hop, winKind)
	winSize = winSize or 256;
	hop = hop or math.floor(winSize / 4);
	winKind = winKind or "hann"
	local win = fft.window(winKind, winSize);
	local nF = floor((# xs - winSize) / hop) + 1;
	local frames = {}
	for fi = 1, nF do
		local s = (fi - 1) * hop;
		local seg = {}
		for i = 1, winSize do
			seg[i] = (xs[s + i] or 0) * win._d:get(i)
		end;
		frames[fi] = fft.rfft(seg)
	end
	return frames
end
--- fft.dct
...
function fft.dct(xs)
	...
	for k = 0, n - 1 do
		local s = 0
		for i = 0, n - 1 do
			s = s + xs[i + 1] * cos(pi * (i + 0.5) * k / n)
		end
		r[k + 1] = s * (k == 0 and sqrt(1 / n) or sqrt(2 / n))
	end;
	return r
end
--- fft.idct
...
function fft.idct(Xs)
	...
	for i = 0, n - 1 do
		local s = 0
		for k = 0, n - 1 do
			s = s + Xs[k + 1] * (k == 0 and sqrt(1 / n) or sqrt(2 / n)) * cos(pi * (i + 0.5) * k / n)
		end
		r[i + 1] = s
	end;
	return r
...
local graph = {}
--- graph.new
-- @param directed type Description
-- @return type Description
...
-- @param data type Description
-- @return type Description
	function G:addNode(id, data)
		if not self.nodes[id] then
			self.nodes[id] = data or {};
			self.adj[id] = {};
			self._n = self._n + 1
		end;
		return self
...
-- @return type Description
	function G:degree(u)
		local d = 0;
		for _ in pairs(self.adj[u] or {}) do
			d = d + 1
		end;
		return d
	end
...
-- @param v type Description
-- @return type Description
	function G:hasEdge(u, v)
		return self.adj[u] and self.adj[u][v] ~= nil
	end
--- G:weight
-- @param u type Description
-- @param v type Description
-- @return type Description
	function G:weight(u, v)
		return self.adj[u] and self.adj[u][v]
	end
--- G:bfs
-- @param src type Description
-- @return type Description
	function G:bfs(src)
		local dist = {};
		local parent = {};
...
-- @return type Description
		local function hpop()
			...
			while true do
				...
				local r = i * 2 + 1;
				local s = i
				if l <= n2 - 1 and heap[l].d < heap[s].d then
					s = l
				end
				if r <= n2 - 1 and heap[r] and heap[r].d < heap[s].d then
					s = r
				end
				if s == i then
					break
...
-- @return type Description
	function G:bellmanFord(src)
		...
		dist[src] = 0
		for _ = 1, self._n - 1 do
			for _, e in ipairs(self.edges) do
				if dist[e.u] < huge and dist[e.u] + e.w < dist[e.v] then
...
-- @return type Description
	function G:floydWarshall()
		...
		for i = 1, n do
			d[i] = {};
			for j = 1, n do
				d[i][j] = i == j and 0 or huge
			end
		end
		for _, e in ipairs(self.edges) do
			d[idx[e.u]][idx[e.v]] = math.min(d[idx[e.u]][idx[e.v]], e.w)
		end
		for k = 1, n do
			for i = 1, n do
				for j = 1, n do
					if d[i][k] < huge and d[k][j] < huge and d[i][k] + d[k][j] < d[i][j] then
						d[i][j] = d[i][k] + d[k][j]
					end
				end
			end
...
--- union
-- @param a2 type Description
-- @param b2 type Description
-- @return type Description
		local function union(a2, b2)
			uf[find(a2)] = find(b2)
		end
		for id in pairs(self.nodes) do
			uf[id] = id
		...
		table.sort(edges, function(a2, b2)
			return a2.w < b2.w
		end)
		local mst = {};
		local total = 0
		for _, e in ipairs(edges) do
			if find(e.u) ~= find(e.v) then
				union(e.u, e.v);
				mst[# mst + 1] = e;
				total = total + e.w
			end
...
-- @return type Description
	function G:topoSort()
		...
		for _, e in ipairs(self.edges) do
			inDeg[e.v] = (inDeg[e.v] or 0) + 1
		end
		local q = {};
		for id, d in pairs(inDeg) do
			if d == 0 then
				q[# q + 1] = id
			end
		end
...
-- @return type Description
		local function sc(v)
			...
			for w in pairs(self.adj[v] or {}) do
				...
				elseif onS[w] then
					low[v] = math.min(low[v], idx_[w])
				end
			end
			if low[v] == idx_[v] then
				local comp = {};
				repeat
					local w = rem(stk);
					onS[w] = false;
...
-- @param heuristic type Description
-- @return type Description
	function G:astar(src, dst, heuristic)
		heuristic = heuristic or function()
			return 0
		end
		local g = {};
		local f = {};
...
-- @return type Description
		local function hpop()
			...
			while true do
				...
				local r = i * 2 + 1;
				local s = i
				if l <= n2 - 1 and open[l] and open[l].f < open[s].f then
					s = l
				end
				if r <= n2 - 1 and open[r] and open[r].f < open[s].f then
					s = r
				end
				if s == i then
					break
...
-- @return type Description
	function G:primMST(src2)
		...
		for _ = 1, self._n do
			local u = nil;
			for id in pairs(self.nodes) do
				if not inMST[id] and (u == nil or key[id] < key[u]) then
					u = id
				end
			end
			if not u then
			...
			for v, w in pairs(self.adj[u] or {}) do
				if not inMST[v] and w < key[v] then
					key[v] = w;
					parent2[v] = u
				end
			end
...
-- @return type Description
	function G:clustering(u)
		...
		for i = 1, k do
			for j = i + 1, k do
				if self:hasEdge(nbrs[i], nbrs[j]) or self:hasEdge(nbrs[j], nbrs[i]) then
					tri = tri + 1
				end
			end
		end
...
-- @return type Description
	function G:pageRank(d, maxI, tol)
		...
		for _ = 1, maxI do
			...
			for _, e in ipairs(self.edges) do
				local deg = self:degree(e.u)
				if deg > 0 then
					np[e.v] = np[e.v] + d * pr[e.u] / deg
				end
			end
			for _, id in ipairs(ids) do
				diff = diff + abs(np[id] - pr[id]);
				pr[id] = np[id]
			end
			if diff < tol then
				break
...
-- @return type Description
	function G:toMatrix()
		...
		local M = Mat.new(n, n)
		for u, nbrs in pairs(self.adj) do
			for v, w in pairs(nbrs) do
				M:set(idx[u], idx[v], w)
			end
		end
		return M, ids
...
-- @param n type Description
-- @return type Description
function graph.cycle(n)
	local G = graph.path(n);
	G:addEdge(n, 1);
	return G
end
--- graph.grid
...
-- @param b type Description
-- @return type Description
function itg.gcd(a, b)
--- g
-- @param x type Description
-- @param y type Description
-- @return type Description
	local function g(x, y)
		return y == 0 and x or g(y, x % y)
	end;
	return g(abs(floor(a)), abs(floor(b)))
end
--- itg.lcm
-- @param a type Description
-- @param b type Description
-- @return type Description
function itg.lcm(a, b)
	return a / itg.gcd(a, b) * b
end
--- itg.isPrime
-- @param n type Description
-- @return type Description
...
-- @param n type Description
-- @return type Description
function itg.euler_phi(n)
	local r = n;
	local tmp = n
	for p = 2, floor(sqrt(tmp)) do
		if tmp % p == 0 then
			r = r - r / p;
			while tmp % p == 0 do
				tmp = floor(tmp / p)
...
-- @param y type Description
-- @return type Description
	local function eg(x, y)
		if y == 0 then
			return x, 1, 0
		end;
		local g, u, v = eg(y, x % y);
		return g, v, u - floor(x / y) * v
	end
	local g, x = eg(a % m, m);
	assert(g == 1, "modinv: no inverse");
	return (x % m + m) % m
end
--- itg.modpow
...
--- itg.crt
-- @param rs type Description
-- @param ms type Description
-- @return type Description
...
-- @param n type Description
-- @return type Description
function itg.nextPrime(n)
	repeat
		n = n + 1
	until itg.millerRabin(n);
	return n
end
--- itg.fibonacci
-- @param n type Description
-- @return type Description
function itg.fibonacci(n)
	local a, b = 0, 1;
	for _ = 1, n do
		a, b = b, a + b
	end;
...
function itg.stirling2(n, k)
	...
	local s = 0;
	for j = 0, k do
		s = s + (j % 2 == 0 and 1 or - 1) * ch(k, j) * (k - j) ^ n
	end
	local fk = 1;
	for i = 1, k do
		fk = fk * i
...
function itg.mobius(n)
	...
	return # f % 2 == 0 and 1 or - 1
end
--- itg.choose
-- @param n type Description
-- @param k type Description
-- @return type Description
function itg.choose(n, k)
	return ch(n, k)
end
--- itg.fact
-- @param n type Description
...
--- itg.continuedFraction
-- @param x type Description
-- @param maxT type Description
-- @return type Description
...
-- @return type Description
	local function impl()
		...
		while true do
			...
			if t2.k == "num" or t2.k == "var" or t2.k == "lb" or t2.k == "lp" or (t2.k == "cmd" and (TRIG2[t2.v] or GREEK2[t2.v] or t2.v == "\\frac" or t2.v == "\\sqrt" or t2.v == "\\sum" or t2.v == "\\int" or t2.v == "\\lim" or t2.v == "\\sin" or t2.v == "\\cos" or t2.v == "\\tan")) then
				a = {
					k = "mul",
					a = a,
					b = postfix()
...
local function evalN(node, env)
	...
	if k == "u" then
		return node.v == "-" and - evalN(node.a, env) or evalN(node.a, env)
	end
	if k == "add" then
		return evalN(node.a, env) + evalN(node.b, env)
	end
	if k == "sub" then
		return evalN(node.a, env) - evalN(node.b, env)
	end
	...
	if k == "frac" then
		return evalN(node.n, env) / evalN(node.d, env)
	end
	if k == "pow" then
		return evalN(node.b, env) ^ evalN(node.e, env)
	end
	if k == "sqrt" then
		local a = evalN(node.a, env);
		return node.deg and a ^ (1 / evalN(node.deg, env)) or sqrt(a)
	end
	if k == "abs" then
		return abs(evalN(node.a, env))
	end
	if k == "eq" then
		return evalN(node.a, env) == evalN(node.b, env) and 1 or 0
	end
	if k == "call" then
		local fn = FNMAP[node.fn];
		return fn and fn(evalN(node.a, env)) or 0
	end
	if k == "sum" then
		local vn = node.var.v;
		local lo_ = floor(evalN(node.lo, env));
		local hi_ = floor(evalN(node.hi, env));
		local s = 0
		for i = lo_, hi_ do
			local e2 = {};
			if env then
				for kk, vv in pairs(env) do
					e2[kk] = vv
				end
			end;
			e2[vn] = i;
			s = s + evalN(node.body, e2)
		end;
		return s
	end
	if k == "int" then
		local vn = node.var and node.var.v or "x"
		local lo_ = node.lo and evalN(node.lo, env) or - huge;
		local hi_ = node.hi and evalN(node.hi, env) or huge
		local f = function(x)
			local e2 = {};
			if env then
				for kk, vv in pairs(env) do
			...
			e2[vn] = x;
			return evalN(node.body, e2)
		end
		if lo_ == - huge or hi_ == huge then
			local s = 0;
			local N = 1000
			for i = 1, N - 1 do
				local x = math.tan(pi / 2 * (2 * i / N - 1));
				local dt = pi / (N * cos(pi / 2 * (2 * i / N - 1)) ^ 2);
				s = s + f(x) * dt
			end;
			return s
		end;
		return calc.romberg(f, lo_, hi_)
	end
	if k == "lim" then
		local vn = node.var.v;
		local to_ = evalN(node.to, env);
		local h = 1e-5
		local f = function(x)
			local e2 = {};
			if env then
...
local function astToStr(n)
	...
	local k = n.k
	if k == "n" then
		return n.v == huge and "\\infty" or tostring(n.v)
	end
	if k == "x" then
		return n.v
	end
	...
	if k == "add" then
		return astToStr(n.a) .. " + " .. astToStr(n.b)
	end
	if k == "sub" then
		return astToStr(n.a) .. " - " .. astToStr(n.b)
	end
	if k == "mul" then
		return astToStr(n.a) .. " \\cdot " .. astToStr(n.b)
	end
	if k == "div" or k == "frac" then
		return "\\frac{" .. astToStr(n.a or n.n) .. "}{" .. astToStr(n.b or n.d) .. "}"
	end
	if k == "pow" then
		return "{" .. astToStr(n.b) .. "}^{" .. astToStr(n.e) .. "}"
	end
	if k == "sqrt" then
		return n.deg and "\\sqrt[" .. astToStr(n.deg) .. "]{" .. astToStr(n.a) .. "}" or "\\sqrt{" .. astToStr(n.a) .. "}"
	end
	if k == "abs" then
		return "\\left|" .. astToStr(n.a) .. "\\right|"
	end
	if k == "call" then
		return "\\" .. n.fn .. "\\left(" .. astToStr(n.a) .. "\\right)"
	end
	if k == "sum" then
		return "\\sum_{" .. astToStr(n.var) .. "=" .. astToStr(n.lo) .. "}^{" .. astToStr(n.hi) .. "}" .. astToStr(n.body)
	end
	if k == "int" then
		local lim = n.lo and "_{" .. astToStr(n.lo) .. "}^{" .. astToStr(n.hi) .. "}" or ""
		return "\\int" .. lim .. astToStr(n.body) .. "\\,d" .. astToStr(n.var)
	end
	if k == "lim" then
		return "\\lim_{" .. astToStr(n.var) .. "\\to " .. astToStr(n.to) .. "}" .. astToStr(n.body)
	end
	return ""
end
--- simplify
...
local function simplify(n)
	...
	if k == "add" then
		...
		if a.k == "n" and a.v == 0 then
			return b
		end
		if b.k == "n" and b.v == 0 then
			return a
		end
		if a.k == "n" and b.k == "n" then
			return {
				k = "n",
				v = a.v + b.v
			}
	...
	if k == "mul" then
		...
		if a.k == "n" and a.v == 1 then
			return b
		end
		if b.k == "n" and b.v == 1 then
			return a
		end
		if a.k == "n" and b.k == "n" then
			return {
	...
	if k == "div" or k == "frac" then
		...
		if a.k == "n" and b.k == "n" and b.v ~= 0 then
			return {
				k = "n",
				v = a.v / b.v
			}
	...
	if k == "pow" then
		...
		if ex.k == "n" and ex.v == 0 then
			return {
				k = "n",
				v = 1
			}
		end
		if ex.k == "n" and ex.v == 1 then
			return base
		end
		if base.k == "n" and base.v == 1 then
			return {
				k = "n",
				v = 1
			}
		end
		if base.k == "n" and ex.k == "n" then
			return {
				k = "n",
				v = base.v ^ ex.v
			}
...
-- @return type Description
	...
	if k == "add" then
		return add(D(n.a), D(n.b))
	end
	if k == "sub" then
		return sub(D(n.a), D(n.b))
	end
	if k == "mul" then
		return add(mul(D(n.a), n.b), mul(n.a, D(n.b)))
	end
	if k == "div" or k == "frac" then
		local f, g = n.a or n.n, n.b or n.d
		return div(sub(mul(D(f), g), mul(f, D(g))), pow(g, N(2)))
	end
	if k == "pow" then
		if n.e.k == "n" then
			local gv = n.e.v;
			return mul(mul(N(gv), pow(n.b, N(gv - 1))), D(n.b))
		end
		return mul(pow(n.b, n.e), add(mul(D(n.e), call("ln", n.b)), mul(n.e, div(D(n.b), n.b))))
	end
	if k == "sqrt" then
		return div(D(n.a), mul(N(2), {
			k = "sqrt",
			a = n.a
		}))
	end
	if k == "abs" then
		return mul(call("sign", n.a), D(n.a))
	end
	if k == "call" then
		local fn = n.fn;
		local f = n.a;
		local df = D(f)
		if fn == "sin" then
			return simplifyN(mul(call("cos", f), df))
		end
		if fn == "cos" then
			return simplifyN(mul(neg(call("sin", f)), df))
		end
		if fn == "tan" then
			return simplifyN(mul(div(N(1), pow(call("cos", f), N(2))), df))
		end
		if fn == "exp" then
			return simplifyN(mul(call("exp", f), df))
		end
		if fn == "ln" then
			return simplifyN(mul(div(N(1), f), df))
		end
		if fn == "sinh" then
			return simplifyN(mul(call("cosh", f), df))
		end
		if fn == "cosh" then
			return simplifyN(mul(call("sinh", f), df))
		end
		if fn == "arcsin" then
			return simplifyN(mul(div(N(1), {
				k = "sqrt",
				a = sub(N(1), pow(f, N(2)))
			}), df))
		end
		if fn == "arctan" then
			return simplifyN(mul(div(N(1), add(N(1), pow(f, N(2)))), df))
		end
		if fn == "sqrt" then
			return simplifyN(mul(div(N(1), mul(N(2), call("sqrt", f))), df))
		end
		return N(0)
	end
...
-- @param src type Description
-- @return type Description
function ltx.parse(src)
	return mkP(ltxTok(src))
end
--- ltx.eval
-- @param src type Description
-- @param env type Description
-- @return type Description
function ltx.eval(src, env)
	return evalN(ltx.parse(src), env)
end
--- ltx.toLatex
-- @param src type Description
-- @return type Description
function ltx.toLatex(src)
	return astToStr(ltx.parse(src))
end
--- ltx.diff
-- @param src type Description
-- @param var type Description
-- @param ord type Description
-- @return type Description
function ltx.diff(src, var, ord)
	var = var or "x";
	ord = ord or 1;
	local ast = ltx.parse(src)
	for _ = 1, ord do
		ast = simplifyN(sdiff(ast, var))
	end
	return astToStr(ast), function(env)
		return evalN(ast, env)
	end
end
--- ltx.diffEval
-- @param src type Description
-- @param var type Description
-- @param xval type Description
-- @return type Description
function ltx.diffEval(src, var, xval)
	var = var or "x";
	local ast = sdiff(ltx.parse(src), var)
	return evalN(ast, {
		[var] = xval
	})
end
...
function ltx.simplify(src, env)
	local ok, v = pcall(ltx.eval, src, env or {});
	return ok and tostring(v) or src
end
mathx.latex = ltx
mathx.ltx = ltx
mathx.PI = pi;
...
mathx.reduce = function(t, f, init)
	local a = init;
	for _, v in ipairs(t) do
		a = f(a, v)
	end;
...
mathx.mean = function(t)
	return mathx.sum(t) / # t
end
mathx.stc = {}
mathx.stc.count = function(t)
	return # t
end
...
mathx.stc.percentile = function(t, p)
	...
	if p <= 0 then
		return mathx.stc.min(t)
	end
	if p >= 1 then
		return mathx.stc.max(t)
	end
	...
	local loVal = a[lo]
	local hiVal = a[hi]
	if lo == hi or hi > n then
		return loVal
	end
	return loVal + (idx - lo) * (hiVal - loVal)
end
mathx.stc.q = mathx.stc.percentile
mathx.stc.q1 = function(t)
	return mathx.stc.percentile(t, 0.25)
end
mathx.stc.q2 = function(t)
	return mathx.stc.percentile(t, 0.5)
end
mathx.stc.q3 = function(t)
	return mathx.stc.percentile(t, 0.75)
end
mathx.stc.median = function(t)
	return mathx.stc.q2(t)
end
mathx.stc.iqr = function(t)
	return mathx.stc.q3(t) - mathx.stc.q1(t)
end
mathx.stc.interquartileRange = mathx.stc.iqr
mathx.stc.quartileDeviation = function(t)
	return mathx.stc.iqr(t) / 2
end
mathx.stc.semiInterquartileRange = mathx.stc.quartileDeviation
mathx.stc.midhinge = function(t)
	return (mathx.stc.q1(t) + mathx.stc.q3(t)) / 2
end
mathx.stc.trimean = function(t)
	return (mathx.stc.q1(t) + 2 * mathx.stc.q2(t) + mathx.stc.q3(t)) / 4
end
mathx.stc.qd = function(t)
	return mathx.stc.quartileDeviation(t)
end
...
mathx.stc.std = function(t, sample)
	return sqrt(mathx.stc.variance(t, sample))
end
...
mathx.stc.binomialPDF = function(n, k, p)
	p = p or 0.5
	if k < 0 or k > n or p < 0 or p > 1 then
		return 0
	end
...
mathx.stc.permutationTest = function(a, b, statFn, samples)
	statFn = statFn or function(x, y)
		return mathx.stc.mean(x) - mathx.stc.mean(y)
	end
	samples = samples or 1000
	local combined = {}
	for i = 1, # a do
		combined[# combined + 1] = a[i]
	end
	for i = 1, # b do
		combined[# combined + 1] = b[i]
	end
...
function BigInt._new(sign, digits)
	...
	if # self.digits == 0 or sign == 0 then
		self.sign = 0;
		self.digits = {}
	end
	return self
...
function BigInt:divmod(other)
	...
	if self.sign == 0 then
		return BigInt._new(0, {}), BigInt._new(0, {})
	end
...
function BigInt:pow(exp)
	exp = tonumber(exp)
	if not exp or exp < 0 or exp ~= floor(exp) then
		error('BigInt exponent must be a non-negative integer')
	end
	local result = BigInt._new(1, {
		1
	})
...
-- @param env type Description
-- @return type Description
function mathx.eval(src, env)
	return ltx.eval(src, env)
end
--- mathx.D
-- @param f type Description
-- @param x type Description
-- @param h type Description
-- @return type Description
function mathx.D(f, x, h)
	return calc.diff(f, x, h)
end
--- mathx.I
-- @param f type Description
-- @param a type Description
-- @param b type Description
-- @return type Description
function mathx.I(f, a, b)
	return calc.romberg(f, a, b)
end
--- mathx.solve
-- @param A type Description
-- @param b type Description
-- @return type Description
function mathx.solve(A, b)
	return la.solve(A, b)
end
--- mathx.gemm
-- @param A type Description
...
-- @param B type Description
-- @return type Description
function mathx.strassen(A, B)
	return blas.strassen(A, B)
end
--- mathx.winograd
-- @param A type Description
-- @param B type Description
-- @return type Description
function mathx.winograd(A, B)
	return blas.winograd(A, B)
end
--- wrap_error
-- @param fn type Description
...
--- wrap_table
-- @param tbl type Description
-- @param prefix type Description
-- @param seen type Description
-- @return type Description
